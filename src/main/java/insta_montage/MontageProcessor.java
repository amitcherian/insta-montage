...
public class MontageProcessor {
    ...
    public void run() {
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            int x = s.borderThickness + col * (tileW + s.borderThickness);
            int y = s.borderThickness + row * (tileH + s.borderThickness);
            ...
            if (s.showLabels) {
                String label = images[i].getTitle();
    ...
    private void drawLabel(ColorProcessor canvas, String text, 
        ...
        int textW = fontSize * text.length() / 2; // rough estimate
        int textH = fontSize;
        ...
        canvas.drawString(text, x, y + textH); // drawString baseline is bottom of text
    }
